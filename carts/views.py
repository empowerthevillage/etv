...
def ajaxRemoveItems(request):
    ...
    item_obj.save()
    item_obj.delete()
    data = {"cartTotal": cart_obj.get_total}
    return JsonResponse(data)
...
def checkout_saved_update(request):
    ...
    device_data = request.POST.get("deviceData")
    order_obj.device_data = device_data
    shipping_address_obj = Address.objects.filter(
...
def checkout_confirm(request):
    try:
        ...
        context = {
            "billing_profile": billing_profile,
            "items_list": cart_items,
            "nonce": gateway.payment_method_nonce.find(
...
def new_charge(request):
    if request.method == "POST":
        ...
        if result.is_success:
            ...
            detail_content = render_to_string(
                "merchandise-admin-email.html",
                {
            ...
            send_mail(
                "New Swag Purchase!",
                str(
                    "A new swag purchase has been successfully processed! Purchaser: "
                    + str(email)
                ),
...
def ticket_nb(request):
    ...
    if result.is_success:
        ...
        for x in tickets:
            ...
            for i in range(batch_size):
                ...
                try:
                    ...
                    bt_obj.purchaser = "%s %s" % (
                        new_ticket.first_name,
                        new_ticket.last_name,
                    )
                    bt_obj.amount = new_ticket.purchase_price
                    bt_obj.url = (
                        "/dashboard/orders/SingleTicket-list/view/%s"
                        % (new_ticket.pk)
                    )
                    bt_obj.save()
                except:
        ...
        donor_obj = Donor.objects.filter(
        ...
        if event.title == "Power Swing Classic Fundraiser":
            recipients = [
                "admin@empowerthevillage.org",
                "ayo@empowerthevillage.org",
                "powerswing@empowerthevillage.org",
            ]
            # recipients = []
        else:
            # recipients = []
        ...
        send_mail(
            ...
            str(
                "A ticket purchase has been successfully processed! Purchaser: "
                + str(email)
            ),
            "etvnotifications@gmail.com",
            recipients,
...
def gallery_sale(request):
    ...
    if result.is_success:
        ...
        send_mail(
            "New Art Purchase!",
            str(
                "A new artwork purchase has been successfully processed! Purchaser: "
                + str(email)
            ),
            "etvnotifications@gmail.com",
            recipients,
...
def full_gallery_sale(request):
    ...
    if result.is_success:
        ...
        confirmation_content = render_to_string(
            "art-email.html",
            {"items": items, "shipped": shipped, "order": order_obj},
        )
        confirmation_plain_text = "View email in browser"
        ...
        send_mail(
            ...
            str(
                "An art piece purchase has been successfully processed! Purchaser: "
                + str(email)
                + " Items: "
                + str(item_string)
            ),
