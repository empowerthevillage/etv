...
mailchimp = Client()
mailchimp.set_config({"api_key": settings.MAILCHIMP_API_KEY, "server": "us7"})
...
def event_detail(request, slug):
    if slug == "friends-of-the-village-art-exhibition":
        return HttpResponseNotFound(
            "Oops! The event you're looking for is no longer available"
        )
    else:
...
def event_ticket_checkout(request, slug):
    ...
    ticket_types = (
        ...
        .filter(sponsorship=False)
        .filter(active=True)
        .order_by("order", "price")
    )
    sponsor_types = (
        TicketType.objects.filter(event=event)
        .filter(sponsorship=True)
        .filter(active=True)
        .order_by("order", "price")
    )
    ...
    for x in ticket_types:
        ...
        if x.qty_limit > 0:
            ...
            quantity_list = range(0, 11)
        cartItem = (
            ticketItem.objects.filter(cart=cart_obj).filter(ticket=x).first()
        )
        dictionary = {
            "type": ticketType,
            "cartItem": cartItem,
    ...
    if billing_profile is not None:
        ...
        if has_card:
            default_card = gateway.payment_method.find(
                billing_profile.default_card.braintree_id
            )
...
def ticket_cart_update(request):
    ...
    item_obj = (
        ticketItem.objects.filter(cart=cart_obj).filter(ticket=type_id).first()
    )
...
def golf(request):
    ...
    bronze = TicketType.objects.filter(
        title="Bronze/Golf Clinic Sponsor"
    ).first()
    ...
    putting = TicketType.objects.filter(
        title="Putting Contest Sponsor"
    ).first()
    beverage = TicketType.objects.filter(
        title="Beverage Station Sponsor"
    ).first()
    ...
    reception = TicketType.objects.filter(
        title="Cocktail Reception Sponsor"
    ).first()
    holeinone = TicketType.objects.filter(title="Hole In One Sponsor").first()
    awards = TicketType.objects.filter(
        title="Awards/Dinner/Banquet Sponsor"
    ).first()

    (cart_obj) = TicketCart.objects.new_or_get(request, event)
    item_list = ticketItem.objects.filter(cart=cart_obj)
    single_quantity = (
        ticketItem.objects.filter(cart=cart_obj).filter(ticket=single).first()
    )
    foursome_quantity = (
        ticketItem.objects.filter(cart=cart_obj)
        .filter(ticket=foursome)
        .first()
    )
    clinic_quantity = (
        ticketItem.objects.filter(cart=cart_obj).filter(ticket=clinic).first()
    )
    mfd_quantity = (
        ticketItem.objects.filter(cart=cart_obj).filter(ticket=mfd).first()
    )

    hole_quantity = (
        ticketItem.objects.filter(cart=cart_obj).filter(ticket=hole).first()
    )
    bronze_quantity = (
        ticketItem.objects.filter(cart=cart_obj).filter(ticket=bronze).first()
    )
    silver_quantity = (
        ticketItem.objects.filter(cart=cart_obj).filter(ticket=silver).first()
    )
    gold_quantity = (
        ticketItem.objects.filter(cart=cart_obj).filter(ticket=gold).first()
    )
    platinum_quantity = (
        ticketItem.objects.filter(cart=cart_obj)
        .filter(ticket=platinum)
        .first()
    )
    diamond_quantity = (
        ticketItem.objects.filter(cart=cart_obj).filter(ticket=diamond).first()
    )

    putting_quantity = (
        ticketItem.objects.filter(cart=cart_obj).filter(ticket=putting).first()
    )
    beverage_quantity = (
        ticketItem.objects.filter(cart=cart_obj)
        .filter(ticket=beverage)
        .first()
    )
    golfcart_quantity = (
        ticketItem.objects.filter(cart=cart_obj)
        .filter(ticket=golfcart)
        .first()
    )
    tee_quantity = (
        ticketItem.objects.filter(cart=cart_obj).filter(ticket=tee).first()
    )
    golfergift_quantity = (
        ticketItem.objects.filter(cart=cart_obj)
        .filter(ticket=golfergift)
        .first()
    )
    reception_quantity = (
        ticketItem.objects.filter(cart=cart_obj)
        .filter(ticket=reception)
        .first()
    )
    holeinone_quantity = (
        ticketItem.objects.filter(cart=cart_obj)
        .filter(ticket=holeinone)
        .first()
    )
    awards_quantity = (
        ticketItem.objects.filter(cart=cart_obj).filter(ticket=awards).first()
    )
...
def art(request):
    ...
    ticket_types = TicketType.objects.filter(event=event)
    single = TicketType.objects.filter(
        title="1-Day Event Pass (Sunday Only)"
    ).first()
    saturday = TicketType.objects.filter(
        title="Saturday VIP Reception"
    ).first()
    twoday = TicketType.objects.filter(title="2-Day Event Pass").first()

    bronze = (
        TicketType.objects.filter(title="Bronze Sponsor")
        .filter(event=event)
        .first()
    )
    silver = (
        TicketType.objects.filter(title="Silver Sponsor")
        .filter(event=event)
        .first()
    )
    gold = (
        TicketType.objects.filter(title="Gold Sponsor")
        .filter(event=event)
        .first()
    )
    platinum = (
        TicketType.objects.filter(title="Platinum Sponsor")
        .filter(event=event)
        .first()
    )
    diamond = (
        TicketType.objects.filter(title="Diamond Sponsor")
        .filter(event=event)
        .first()
    )
    ...
    saturday_quantity = (
        ticketItem.objects.filter(cart=cart_obj)
        .filter(ticket=saturday)
        .first()
    )
    twoday_quantity = (
        ticketItem.objects.filter(cart=cart_obj).filter(ticket=twoday).first()
    )
...
def ticket(request, ticket_id):
    ...
    email = object.email
    if request.user.is_authenticated:
        related = SingleTicket.objects.filter(email=email).filter(
            checked_in=False
        )
        if len(related) == 0:
            related = [object]
        context = {"related": related, "object": object}
    else:
        context = {
            "object": object,
        }
...
def art_checkout(request):
    ...
    (cart_obj) = TicketCart.objects.new_or_get(request, event)
    item_list = ticketItem.objects.filter(cart=cart_obj).filter(event=event)
    donations = ticketDonation.objects.filter(cart=cart_obj).filter(
        event=event
    )
    single_quantity = (
        ticketItem.objects.filter(cart=cart_obj).filter(ticket=single).first()
...
def full_gallery_home(request):
    cart_obj, created = FullGalleryCart.objects.new_or_get(request)
    items = FullGalleryItem.objects.filter(active=True).order_by(
        "sold", "artist", "title"
    )[:12]
    artists = Artist.objects.filter(active=True).order_by("name")
    filter = GalleryFilter(request.GET, queryset=items)
...
def gallery_get_next(request):
    ...
    if page.has_next():
        html = render_to_string(
            "gallery-next.html", {"page": page, "error_msg": None}
        )
        ...
        html = render_to_string(
            "gallery-next.html",
            {"error_msg": True, "message": "End of Gallery"},
        )
...
def gallery_search_available(request):
    ...
    if title and availability is not None:
        items = FullGalleryItem.objects.filter(
            Q(title__icontains=title) & Q(sold=availability) & Q(active=True)
        )

    elif title:
        items = FullGalleryItem.objects.filter(
            Q(title__icontains=title) & Q(active=True)
        )

    elif availability is not None:
        items = FullGalleryItem.objects.filter(
            Q(sold=availability) & Q(active=True)
        )

    if len(items) > 0:
        p = Paginator(items, 24)
        html = render_to_string(
            "gallery-next.html", {"page": p.get_page(1), "error_msg": None}
        )
        return HttpResponse(html)
    else:
        html = render_to_string(
...
def gallery_search(request):
    ...
    if title and artist:
        ...
        items = FullGalleryItem.objects.filter(
            Q(artist__icontains=artist) & Q(active=True)
        )
...
def new_checkin(request):
    f = TicketFilter(
        request.GET, queryset=SingleTicket.objects.filter(checked_in=False)
    )
    items = f.qs
    context = {
        "f": f,
...
def pitch_rules_agreement(request):
    if request.method == "POST":
        obj = Signature()
        obj.purpose = (
            "Agreed to 2024 Empowerment Expo Village Ventures Rules & Terms"
        )
...
def process_silent_auction_purchase(request):
    if request.method == "POST":
        ...
        result = gateway.transaction.sale(
            {
                ...
                "custom_fields": {
                    "memo": "Power Swing Silent Auction Purchase - %s"
                    % (item_obj)
                },
                "options": {
        ...
        if result.is_success:
            merge_fields = {"FNAME": str(first_name), "ETVAMOUNT": str(total)}
            ...
            send_mail(
                ...
                str(
                    "A new silent auction for %s has been received from %s %s through www.empowerthevillage.org!"
                    % (item_obj, first_name, last_name)
                ),
                "etvnotifications@gmail.com",
                [
                    "admin@empowerthevillage.org",
                    "chandler@eliftcreations.com",
                    "ayo@empowerthevillage.org",
                ],
                # ['chandler@eliftcreations.com'],
                fail_silently=True,
            )
            ...
            data = {
                "status": "success",
                "html": render_to_string(
                    "silent-auction-buy-now-success.html",
                    context={
                        "purchase": order_obj,
...
def free_registration(request, slug):
    if request.method == "POST":
        try:
            ...
            reg_obj_temp = FreeRegistrationTemplate.objects.filter(
                event=event
            ).first()
            data = request.POST
            ...
            try:
                ...
                registrant_subject = (
                    "Free Registration for ETV's %s Confirmed!" % (event)
                )
                registrant_content = render_to_string(
                    "free-reg-email.html", {}
                )
                registrant_plain_text = "View email in browser"
                send_mail(
                    registrant_subject,
                    registrant_plain_text,
                    from_email,
                    [str(reg_obj.email)],
                    html_message=registrant_content,
                )

                confirmation_subject = "New %s Free Registration!" % (
                    reg_obj_temp
                )

                confirmation_content = render_to_string(
                    "new-free-reg.html",
                    {"obj": reg_obj, "template": reg_obj_temp},
                )
                confirmation_plain_text = "View email in browser"
                send_mail(
                    confirmation_subject,
                    confirmation_plain_text,
                    from_email,
                    [
                        "chandler@eliftcreations.com",
                        "ayo@empowerthevillage.org",
                    ...
                    html_message=confirmation_content,
                )
            except:
                pass
            ...
            sweetify.success(
                ...
                title="Success!",
                icon="success",
                text="You're registered for the Expo!",
                button="OK",
                timer=20000,
            )
            ...
            sweetify.error(
                "Please make sure all required fields are valid and try again"
            )
    else:
        event = Event.objects.filter(slug=slug).first()
