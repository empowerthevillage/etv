...
def event_ticket_checkout(request, slug):
    event = Event.objects.get(slug=slug)
    ticket_types = TicketType.objects.filter(event=event).filter(sponsorship=False).order_by('order', 'price')
    sponsor_types = TicketType.objects.filter(event=event).filter(sponsorship=True).order_by('order', 'price')
...
def full_gallery_home(request):
    cart_obj, created = FullGalleryCart.objects.new_or_get(request)
    items = FullGalleryItem.objects.all().order_by('order', 'artist', '-sold', 'price')
    artists = Artist.objects.all().order_by('name')
    auction_items = AuctionItem.objects.all()
    p = Paginator(items, 6)
    filter = GalleryFilter(request.GET, queryset=items)
