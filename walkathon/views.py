...
def walker_detail(request, walker):
    ...
    try:
        ...
        walker_list = ""
        donation_list = WalkerDonation.objects.filter(
            walker=walker_obj
        ...
        walker_type = "org"
        donation_list = OrgDonation.objects.filter(
            organization=walker_obj
        ).filter(displayed=True)
    context = {
...
def org_walker_detail(request, org, walker):
    ...
    walker_type = "individual"
    walker_list = ""
    donation_list = WalkerDonation.objects.filter(walker=walker_obj).filter(
        complete=True
    )
    context = {
        "title": "Support %s" % (walker_obj),
...
def walker_registration(request):
    if request.method == "POST":
        ...
        if result.is_success:
            ...
            walker_obj.emergency_contact_phone = data[
            ...
            try:
                ...
                pass
            try:
                walker_obj.donation_goal = float(
                    request.POST["fundraising-goal"].replace(",", "")
                )
            except:
                pass
            walker_obj.save()
...
def org_registration(request):
    if request.method == "POST":
        org, created = Organization.objects.get_or_create(
            title=request.POST["organization"]
        )
        if created == True:
            ...
            context = {"already_registered_popup": True}
    ...
    if request.method == "POST":
        ...
        if result.is_success:
            ...
            data = {
                "status": "success",
                "html": render_to_string(
                    "sponsor-success.html",
                    context={
                        "sponsorship": spons_obj,
                        "payment_method": result.transaction.credit_card_details,
                        "total": total,
                    },
                ),
